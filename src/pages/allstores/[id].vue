<template>
  <!-- Banner Section -->
  <v-col cols="12" sm="12" class="ssbnimg">
    <v-container>
      <v-col>
        <v-row justify="center">
          <v-col cols="12" sm="12">
            <div class="ssbannertitle">
              <h1 class="ssbannertitle1">{{ singlestore.name }}</h1>
            </div>
          </v-col>
        </v-row>
      </v-col>
  ...
  <v-container>
    <v-col cols="12" sm="12" class="mt-16">
      <v-row justify="center">
        <v-col cols="12" sm="12">
          <v-row id="sspoint01">
            <v-col cols="12" sm="3" class="sstore01">
              <v-img
                :aspect-ratio="1"
                class="bg-white"
                id="sstoreimg"
            ...
            <v-col cols="12" sm="9" class="sstore02">
              <v-card>
                <div id="sstorewd">
                  <p id="sstoretitle">{{ singlestore.name }}</p>
                  <div class="mt-8 sstoretext">
                    <p>
                      {{ singlestore.description }}</p>
                  </div>
                </div>
              </v-card>
            </v-col>
    ...
    <v-col cols="12" sm="12">
      <v-row>
        <v-col cols="12" sm="8">
          <div class="ssservice" id="sspoint02">
            <h1>服務項目</h1>

            <v-carousel hide-delimiters class="my-6">
  <v-carousel-item
    src="singlestore.slide"
    cover
  ></v-carousel-item>
</v-carousel>

            <p class="sstoretext">
              多款家用好物任意挑選 — 原本生活帶你享受生活的美好，質感設計讓使用上除了美觀外，更能展現品味，享受「好生活」 消費滿額立享免運優惠，超取、宅配任君選擇，歡慶3周年，立享獨家優惠！多款家用好物任意挑選 — 原本生活帶你享受生活的美好，質感設計讓使用上除了美觀外，更能展現品味，享受「好生活」 消費滿額立享免運優惠，超取、宅配任君選擇，歡慶3周年，立享獨家優惠！多款家用好物任意挑選 — 原本生活帶你享受生活的美好，質感設計讓使用上除了美觀外，更能展現品味，享受「好生活」 消費滿額立享免運優惠，超取、宅配任君選擇，歡慶3周年，立享獨家優惠！
              多款家用好物任意挑選 — 原本生活帶你享受生活的美好，質感設計讓使用上除了美觀外，更能展現品味，享受「好生活」 消費滿額立享免運優惠，超取、宅配任君選擇，歡慶3周年，立享獨家優惠！多款家用好物任意挑選 — 原本生活帶你享受生活的美好，質感設計讓使用上除了美觀外，更能展現品味，享受「好生活」 消費滿額立享免運優惠，超取、宅配任君選擇，歡慶3周年，立享獨家優惠！多款家用好物任意挑選 — 原本生活帶你享受生活的美好，質感設計讓使用上除了美觀外，更能展現品味，享受「好生活」 消費滿額立享免運優惠，超取、宅配任君選擇，歡慶3周年，立享獨家優惠！
            </p>
          </div>
          <div class="ssservice mt-6" id="sspoint03">
            <v-row>
              <v-col cols="12" sm="6">
        ...
        <v-col cols="12" sm="4">
          <div class="ssservice" id="sspoint04">
            <div>
              <h1>商家資訊</h1>
              <div class="my-6">
                <iframe
...
const load = async () => {
  try {
    const { data } = await api.get('/singlestore/' + route.params.id)
    singlestore.value._id = data.result._id
    singlestore.value.name = data.result.name
    singlestore.value.banner = data.result.banner
    singlestore.value.description = data.result.description
    singlestore.value.image = data.result.image
    singlestore.value.slide = data.result.slide
    singlestore.value.category = data.result.category

    document.title = '創業趣 | ' + singlestore.value.name
  } catch (error) {
    console.log(error)
    createSnackbar({
...
export default {
  ...
  methods: {
    ...
    scrollToSspoint01 () {
      const element = document.getElementById('sspoint01')
      if (element) {
        window.scrollTo({
    ...
    scrollToSspoint02 () {
      const element = document.getElementById('sspoint02')
      if (element) {
        window.scrollTo({
          top: element.offsetTop - 160,
          behavior: 'smooth'
    ...
    scrollToSspoint03 () {
      const element = document.getElementById('sspoint03')
      if (element) {
        window.scrollTo({
          top: element.offsetTop - 160,
          behavior: 'smooth'
    ...
    scrollToSspoint04 () {
      const element = document.getElementById('sspoint04')
      if (element) {
        window.scrollTo({
          top: element.offsetTop - 160,
          behavior: 'smooth'
...
.ssbnimg {
  background: url(singlestore.banner);
